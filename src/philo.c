...
/*   Updated: 2021/11/22 12:18:14 by seongjki         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_philo(t_philo_info *philo_info)
{
	...
	while (idx < philo_info->rule.num_of_philo)
	{
		...
		philo_info->philo[idx].lfork = &philo_info->forks[idx];
		philo_info->philo[idx].rule = &philo_info->rule;
		philo_info->philo[idx].time = &philo_info->time;
		philo_info->philo[idx].print_mutex = &philo_info->print_mutex;
		if (idx == philo_info->rule.num_of_philo - 1)
			philo_info->philo[idx].rfork = &philo_info->forks[0];
		else
			philo_info->philo[idx].rfork = &philo_info->forks[idx + 1];
		idx++;
	}
}
